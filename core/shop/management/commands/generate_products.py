...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--count", type=int, default=10, help="تعداد محصولات جعلی برای تولید"
        )
    ...
    def handle(self, *args, **options):  # ✅ Correct level
        ...
        image_folder = os.path.join(
            settings.BASE_DIR, "shop", "management", "commands", "images"
        )
        image_files = [
            f
        ...
        if not image_files:
            self.stdout.write(
                self.style.ERROR(f"No image files found in: {image_folder}")
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully created {created_count} fake products.")
        )
